  ...
  if (
    ...
    ranges.some((range) => {
      ...
      return (
        ...
        item.from < 0 ||
        (item.to !== undefined &&
          (typeof item.to !== 'number' || !Number.isFinite(item.to) || item.to < item.from)) ||
...
export const GET: APIRoute = async ({ url }) => {
  try {
    ...
    if (query && query.length < 3)
      return new Response(JSON.stringify({ error: 'Search requires at least 3 characters.' }), {
        status: 400,
    ...
    if (
      ...
      (error instanceof Error && error.message === 'Invalid magnitude filters.')
    ) {
      return Response.json(
        { error: 'Invalid magnitude filters.' },
