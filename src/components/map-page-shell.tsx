...
function MapPageContent({ children, operatorEmail, operatorDisplayName }: Props) {
  ...
  const [filtered, setFiltered] = React.useState<EventState>({
    events: [],
    hasMore: false,
    atLimit: false,
    loadingMore: false,
      ...
      try {
        ...
        document.dispatchEvent(
          new CustomEvent(EARTHQUAKE_RENDER_EVENTS_EVENT, {
            detail: { events: page.events, fitBounds: true },
          }),
        );
      } catch (error) {
        if (searchRequest.current !== requestId) return;
        ...
        setSearchError(
          error instanceof Error ? error.message : 'Could not search earthquake events.',
        );
      }
    }, 300);
  ...
  React.useEffect(() => {
    document.dispatchEvent(
      new CustomEvent(EARTHQUAKE_RENDER_EVENTS_EVENT, { detail: { events: visibleEvents } }),
    );
  }, [visibleEvents]);
  ...
  async function loadMoreEvents() {
    ...
    try {
      ...
      setSearchAtLimit(page.atLimit);
      document.dispatchEvent(
        new CustomEvent(EARTHQUAKE_RENDER_EVENTS_EVENT, { detail: { events } }),
      );
    } catch (error) {
      if (searchRequest.current === requestId) {
        setSearchError(
          error instanceof Error ? error.message : 'Could not load more earthquake events.',
        );
      }
    } finally {
      searchLoadingMoreRef.current = false;
  ...
  return (
    <>
      ...
      <div
        className={cn(
          'fixed inset-0 z-40 overflow-hidden bg-sidebar transition-[width] md:static md:z-auto md:block',
          filtersOpen || forecastEvent ? 'w-full md:w-80 md:border-r' : 'pointer-events-none w-0',
        )}
      >
        {filtersOpen ? (
