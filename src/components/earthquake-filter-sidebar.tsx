...
import { MAGNITUDE_RANGE_OPTIONS, type MagnitudeRange } from '@/lib/magnitude-ranges';
...
function sameRange(left: MagnitudeRange, right: MagnitudeRange) {
  return (
    left.from === right.from && left.to === right.to && left.upperExclusive === right.upperExclusive
  );
}
...
export function EarthquakeFilterSidebar({ filters, onApply, onClose }: Props) {
  ...
  function toggleMagnitude(range: MagnitudeRange, checked: boolean) {
    ...
    setDraft({
      ...draft,
      events: { ...draft.events, magnitude: magnitude.length ? magnitude : null },
    });
  }
  ...
  function setRange(kind: 'depth' | 'date', key: keyof FilterRange, value: string) {
    setDraft({
      ...
      events: {
        ...draft.events,
        [kind]: {
          from: draft.events[kind]?.from ?? '',
          to: draft.events[kind]?.to ?? '',
          [key]: value,
        },
      },
    });
  ...
  const depthInvalid = Boolean(
    draft.events.depth?.from &&
    draft.events.depth.to &&
    Number(draft.events.depth.from) > Number(draft.events.depth.to),
  );
  const dateInvalid = Boolean(
    draft.events.date?.from &&
    draft.events.date.to &&
    draft.events.date.from > draft.events.date.to,
  );
  ...
  return (
    <aside
      ...
      className="flex h-full min-h-0 w-full flex-col bg-sidebar text-sidebar-foreground md:w-80"
    >
      ...
      <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-4 text-sm">
            ...
            <div className="grid grid-cols-2 gap-2">
              {MAGNITUDE_RANGE_OPTIONS.map((option) => (
                <label key={option.value} className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    checked={(draft.events.magnitude ?? []).some((range) =>
                      sameRange(range, option.range),
                    )}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <Input
                type="number"
                min="0"
                max={draft.events.depth?.to || undefined}
                aria-invalid={depthInvalid}
                placeholder="From"
                value={draft.events.depth?.from ?? ''}
                onChange={(event) => setRange('depth', 'from', event.target.value)}
              />
              <Input
                type="number"
                min={draft.events.depth?.from || '0'}
                aria-invalid={depthInvalid}
                placeholder="To"
                value={draft.events.depth?.to ?? ''}
                onChange={(event) => setRange('depth', 'to', event.target.value)}
              />
            </div>
            {depthInvalid ? (
              <p role="alert" className="mt-1 text-xs text-destructive">
                Depth to must be at least depth from.
              </p>
            ) : null}
          </div>
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <Input
                type="date"
                max={draft.events.date?.to || undefined}
                aria-label="Date from"
                aria-invalid={dateInvalid}
                value={draft.events.date?.from ?? ''}
                onChange={(event) => setRange('date', 'from', event.target.value)}
              />
              <Input
                type="date"
                min={draft.events.date?.from || undefined}
                aria-label="Date to"
                aria-invalid={dateInvalid}
                value={draft.events.date?.to ?? ''}
                onChange={(event) => setRange('date', 'to', event.target.value)}
              />
            </div>
            {dateInvalid ? (
              <p role="alert" className="mt-1 text-xs text-destructive">
        ...
        <ForecastFilterFields
          value={draft.forecasts}
          onChange={(forecasts) => setDraft({ ...draft, forecasts })}
        />
      </div>
      ...
      <footer className="border-t border-sidebar-border p-2">
        <div className="grid grid-cols-2 gap-2">
          ...
          <Button type="button" disabled={depthInvalid || dateInvalid} onClick={apply}>
            Apply filters
          </Button>
        </div>
      </footer>
