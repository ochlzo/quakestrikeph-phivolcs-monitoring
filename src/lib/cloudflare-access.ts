...
export async function getAccessUser(
  ...
  const developmentEmail = allowDevelopmentIdentity
    ? environment.developmentEmail?.trim()
    : undefined;
  if (developmentEmail) {
...
export function isTrustedMutationOrigin(
  ...
  const expectedOrigin =
    portalOrigin?.trim() || (allowRequestOriginFallback ? new URL(request.url).origin : '');
