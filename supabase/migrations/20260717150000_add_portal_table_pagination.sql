create function public.get_portal_forecast_page(
...
create function public.get_portal_event_page(
  query_text text default null,
  magnitude_ranges jsonb default null,
  depth_from double precision default null,
  depth_to double precision default null,
...
create function public.get_portal_audit_log_page(
  query_text text default null,
...
as $$
  with source as materialized (
    select
      log.id,
      log.user_email,
      profile.display_name as operator_display_name,
      log.path,
      log.method,
      log.created_at,
      log.metadata,
      coalesce(nullif(log.metadata->>'action', ''), log.method || ' request') as action
    from public.audit_logs as log
    left join public.operator_profiles as profile on lower(profile.email) = lower(log.user_email)
  ), filtered as materialized (
    select *
    from source
    where (
        nullif(trim(query_text), '') is null
        or position(lower(trim(query_text)) in lower(concat_ws(' ', user_email, operator_display_name, path))) > 0
      )
      and (actions is null or cardinality(actions) = 0 or action = any(actions))
      and (date_from is null or (created_at at time zone 'Asia/Manila')::date >= date_from)
      and (date_to is null or (created_at at time zone 'Asia/Manila')::date <= date_to)
  ), page_rows as (
    select *
    from filtered
  ...
  select jsonb_build_object(
    'total', (select count(*) from filtered),
    'rows', coalesce((
      select jsonb_agg(to_jsonb(page_rows) order by page_rows.created_at desc, page_rows.id desc)
      from page_rows
    ), '[]'::jsonb),
    'actions', coalesce((
      select jsonb_agg(action order by action)
      from (select distinct source.action from source) as available_actions
    ), '[]'::jsonb)
  );
$$;

revoke all on function public.get_portal_forecast_page(
  text, text[], jsonb, double precision, double precision, date, date,
  text[], text[], double precision, integer, integer
) from public;
revoke all on function public.get_portal_event_page(
  text, jsonb, double precision, double precision, date, date, integer, integer
) from public;
revoke all on function public.get_portal_audit_log_page(
  text, text[], date, date, integer, integer
) from public;

grant execute on function public.get_portal_forecast_page(
  text, text[], jsonb, double precision, double precision, date, date,
  text[], text[], double precision, integer, integer
) to service_role;
grant execute on function public.get_portal_event_page(
  text, jsonb, double precision, double precision, date, date, integer, integer
) to service_role;
grant execute on function public.get_portal_audit_log_page(
  text, text[], date, date, integer, integer
) to service_role;
